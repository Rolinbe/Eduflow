...
export default function ChatPanel() {
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] bg-white rounded-xl shadow-card overflow-hidden border border-gray-100">
      {callState.isActive && !callState.isIncoming && <VoiceCall />}

      {/* Sidebar: conversations list */}
      <div className={`w-80 border-r border-gray-100 flex flex-col ${activeConversationId ? 'hidden lg:flex' : 'flex'}`}>
        <div className="p-4 border-b border-gray-100">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-bold text-gray-900">Messages</h2>
            <button
              onClick={() => setShowNewChat(!showNewChat)}
              className="w-9 h-9 bg-primary-50 text-primary-600 rounded-lg flex items-center justify-center hover:bg-primary-100"
            >
          ...
          <input
            ...
            className="w-full px-4 py-2 bg-gray-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
          />
        </div>
          ...
          {showNewChat ? (
            <div className="p-2">
              <div className="flex items-center justify-between px-3 py-2 mb-2">
                <p className="text-xs font-semibold text-gray-500 uppercase">Nouvelle conversation</p>
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-xs text-gray-400 mt-1">Cliquez + pour commencer</p>
            </div>
          )}
        </div>
      </div>
        ...
        {activeConversationId && activeConv ? (
          <>
            <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-white">
              ...
              <button
                ...
                className="w-9 h-9 bg-green-50 text-green-600 rounded-lg flex items-center justify-center hover:bg-green-100"
                title="Appel vocal"
              >
            ...
            <div className="p-4 border-t border-gray-100 bg-white">
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="flex-1 px-4 py-3 bg-gray-50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 border border-gray-200"
                />
                <button
                  onClick={sendMessage}
                  disabled={!inputValue.trim()}
                  className="w-11 h-11 bg-primary-500 text-white rounded-xl flex items-center justify-center hover:bg-primary-600 transition-colors disabled:opacity-50"
                >
          ...
          <div className="flex-1 flex items-center justify-center bg-gray-50">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 mt-1">Sélectionnez une conversation ou commencez-en une nouvelle</p>
            </div>
          </div>
        )}
      </div>
